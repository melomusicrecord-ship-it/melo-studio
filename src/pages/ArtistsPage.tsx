...
export function ArtistsPage({
  ...
  // Synchronize viewMode if subFilter changes from top navigation
  useEffect(() => {
    if (subFilter === 'budget') {
      setViewMode('budget');
  ...
  // Artistas que possuem projetos no estúdio
  ...
  // Se o usuário estiver na visualização de Orçamento (pelo botão ou subfiltro)
  if (viewMode === 'budget') {
    return (
      <div className="space-y-4 animate-in fade-in duration-200">
        <div className="flex items-center justify-between pb-1">
          <button
            ...
            className="px-3.5 py-1.5 rounded-xl bg-zinc-900 hover:bg-zinc-800 border border-zinc-750 text-zinc-300 hover:text-white flex items-center gap-1.5 text-xs font-semibold transition-all shadow"
          >
            <span>← Voltar para Catálogo de Artistas</span>
          </button>
        </div>
        ...
        <ArtistBudgetManager
          projects={projects}
          artists={artists}
          onSaveProject={onSaveProject || (async () => {})}
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="px-4 py-2 rounded-xl bg-gradient-to-r from-amber-500/20 via-amber-600/15 to-amber-700/20 hover:from-amber-500/30 hover:to-amber-600/30 border border-amber-500/40 text-amber-300 font-bold text-xs flex items-center gap-2 shadow-lg active:scale-95 transition-all shrink-0 group"
            title="Ver orçamento de todos os artistas que têm projetos com o estúdio"
          >
            <DollarSign className="w-4 h-4 text-amber-400 group-hover:scale-110 transition-transform" />
            <span>Orçamento dos Artistas</span>
            <span className="px-2 py-0.5 rounded-full bg-amber-500/25 text-amber-200 text-[10px] font-mono border border-amber-500/40 font-bold">
              {artistsWithProjects.length} com projeto
            </span>
          </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredArtists.map((artist) => {
          // Check projects for this artist
          ...
          artistProjects.forEach((p) => {
            const t = p.budget?.totalAmount ?? 200;
            const pd = p.budget?.paidAmount ?? (p.budget?.paymentStatus === 'Completo' ? t : 0);
            totalBudget += t;
            totalPaid += pd;
          ...
          return (
            ...
            >
              <div>
                ...
                {/* Card de Orçamento Rápido para Artistas com Projeto */}
                {artistProjects.length > 0 ? (
                  <button
                    onClick={() => setViewMode('budget')}
                    className="w-full mb-3 p-2.5 rounded-xl bg-gradient-to-br from-zinc-900 to-zinc-950 border border-amber-500/30 hover:border-amber-500/60 text-left transition-all group/btn"
                    title="Clique para gerenciar o orçamento e entrega deste artista"
                  >
                    <div className="flex items-center justify-between text-[11px] mb-1">
                      <span className="text-amber-400 font-bold flex items-center gap-1">
                        <DollarSign className="w-3 h-3 text-amber-400" />
                        {pct}% Pago ({totalPaid}€ / {totalBudget}€)
                      </span>
                      <span
                      ...
                      >
                        {allDelivered ? 'Música Entregue ✅' : 'Música Pendente ⏳'}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center justify-between text-[10px] text-zinc-500">
                      <span>{artistProjects.length} projeto(s) ativo(s)</span>
                      <span className="text-amber-400/80 group-hover/btn:underline">Ver Orçamento →</span>
                    </div>
                  </button>
                ) : (
                  <div className="mb-3 py-1.5 px-2.5 rounded-lg bg-zinc-900/60 border border-zinc-850 text-[10px] text-zinc-500 flex items-center justify-between">
                    <span>Sem projetos no estúdio</span>
                    <span className="text-zinc-600">Apenas cadastro</span>
                  </div>
                )}
